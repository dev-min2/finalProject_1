...
let reviewDAO = {
    ...
    selectReviewListQuery: async function (userNo, pageNo) {
        ...
        const selectReviewListQuery =
            `SELECT DISTINCT r.review_no, p.payment_no, a.product_no,a.product_name, a.product_price, p.payment_date
            FROM user u
            JOIN payment p ON u.user_no = p.user_no
            JOIN review r ON p.payment_no = r.payment_no
            JOIN product a ON r.product_no = a.product_no
            WHERE u.user_no = ?
